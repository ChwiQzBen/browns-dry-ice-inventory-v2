...
#🎨 KPI SPARKLINES
def kpi_sparklines(df, kpis, constants):
    """
    Display interactive KPI cards with sparkline mini-charts
    
    Args:
        df: DataFrame with historical order data
        kpis: Dictionary with KPI values
        constants: Constants object with pricing and other parameters
    """
    if df is None or df.empty:
        st.info("No data available for KPI sparklines")
        return
    
    # Create columns for KPIs
    col1, col2, col3, col4 = st.columns(4)
    
    # Prepare monthly data for sparklines
    try:
        df_monthly = df.copy()
        df_monthly['Date'] = pd.to_datetime(df_monthly['Date'])
        df_monthly['Month'] = df_monthly['Date'].dt.to_period('M')
        monthly_data = df_monthly.groupby('Month')['Order_Quantity_kg'].sum().reset_index()
        monthly_values = monthly_data['Order_Quantity_kg'].values.tolist()
    except Exception as e:
        # Fallback: generate synthetic data if actual data fails
        monthly_values = [0] * 6
    ...
    with col1:
        ...
        fig1.add_trace(go.Scatter(
            ...
            y=monthly_values if monthly_values else [0],
            mode='lines',
        ...
        st.metric(
            ...
            delta=f"{kpis.get('order_change', 0):+,}" if kpis.get('order_change') else None
        )
        st.plotly_chart(fig1, use_container_width=True, config={'displayModeBar': False})
        st.caption(f"Monthly trend • {len(monthly_values)} months")
    
    # KPI 2: Total Volume
    with col2:
        total_volume = kpis.get('total_volume', 0)
        
        # Create sparkline for volume
        fig2 = go.Figure()
        ...
        st.metric(
            ...
            delta=f"{kpis.get('volume_change', 0):+,.0f} kg" if kpis.get('volume_change') else None
        )
        st.plotly_chart(fig2, use_container_width=True, config={'displayModeBar': False})
        st.caption(f"Monthly volume trend")
    
    # KPI 3: Monthly Savings
    with col3:
        monthly_savings = kpis.get('monthly_savings', 0)
        
        # Create sparkline for savings
        savings_values = [v * 0.8 + (i * 0.05 * v) for i, v in enumerate(monthly_values[:6])] if monthly_values else [0] * 6
        fig3 = go.Figure()
        fig3.add_trace(go.Scatter(
            x=list(range(len(savings_values))),
            y=savings_values if savings_values else [0],
            mode='lines',
        ...
        st.metric(
            ...
            value=f"KSh {monthly_savings:,.0f}",
            delta=f"{kpis.get('savings_percent', 0):+.1f}%" if kpis.get('savings_percent') else None
        )
        st.plotly_chart(fig3, use_container_width=True, config={'displayModeBar': False})
        st.caption(f"Cost savings trend")
    
    # KPI 4: EOQ Efficiency
    with col4:
        eoq = kpis.get('eoq', 0)
        current_orders = kpis.get('order_frequency', 0)
        eoq_orders = kpis.get('eoq_orders', 0)
        efficiency = (eoq_orders / current_orders * 100) if current_orders > 0 else 0
        
        # Create sparkline for efficiency
        efficiency_values = [efficiency * (0.5 + (i / (len(monthly_values) * 2))) for i in range(len(monthly_values))] if monthly_values else [0] * 6
        fig4 = go.Figure()
        fig4.add_trace(go.Scatter(
            x=list(range(len(efficiency_values))),
            y=efficiency_values if efficiency_values else [0],
            mode='lines',
        ...
        st.metric(
            label="🎯 EOQ Efficiency",
            value=f"{efficiency:.1f}%",
            delta=f"{kpis.get('efficiency_change', 0):+.1f}%" if kpis.get('efficiency_change') else None
        )
        st.plotly_chart(fig4, use_container_width=True, config={'displayModeBar': False})
        st.caption(f"Order optimization trend")  
...
def main():
    ...
    # Add interactive KPI cards with sparklines
    if not df.empty and len(df) > 0:
        try:
            # Prepare additional KPI data
            # Get monthly data for sparklines
            df_temp = df.copy()
            df_temp['Date'] = pd.to_datetime(df_temp['Date'])
            df_temp['Month'] = df_temp['Date'].dt.to_period('M')
            monthly_trend = df_temp.groupby('Month')['Order_Quantity_kg'].sum().reset_index()
            
            # Calculate additional KPI values
            monthly_savings = annual_transport_savings / 12 if 'annual_transport_savings' in locals() else 0
            eoq_monthly_orders = adjusted_demand / eoq if eoq > 0 else 0
            current_monthly_orders = kpis.get('order_frequency', 0)
            
            # Update kpis with additional values for sparklines
            kpis.update({
                'monthly_savings': monthly_savings,
                'eoq_efficiency': (eoq_monthly_orders / current_monthly_orders * 100) if current_monthly_orders > 0 else 0,
                'total_annual_cost': total_annual_spending if 'total_annual_spending' in locals() else 0,
                'holding_cost': annual_holding_cost if 'annual_holding_cost' in locals() else 0,
                'transport_cost': annual_transport_cost if 'annual_transport_cost' in locals() else 0,
                'order_change': 0,  # You can calculate this if you have previous period data
                'volume_change': 0,  # You can calculate this if you have previous period data
                'savings_percent': (monthly_savings / (current_monthly_orders * constants.TRANSPORT_COST) * 100) if current_monthly_orders > 0 else 0,
                'previous_total_orders': kpis.get('total_orders', 0),  # Placeholder
                'previous_volume': kpis.get('total_volume', 0),  # Placeholder
                'previous_efficiency': 50,  # Placeholder
                'eoq_orders': eoq_monthly_orders,
                'eoq': eoq
            })
            
            # Display the KPI Sparklines
            kpi_sparklines(df, kpis, constants)
            st.markdown("---")
            
        except Exception as e:
            st.warning(f"Could not display KPI sparklines: {e}")
    else:
        st.info("📊 No order data available for KPI sparklines")  
